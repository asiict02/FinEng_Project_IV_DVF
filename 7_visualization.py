...
7_visualization.py — Visualization of Results
  ...
  4. OOS Loss Heatmap (2×2 per model, one subplot per model)
...
# ── Load data ──────────────────────────────────────────────────────────────────
df_iv     = pd.read_csv(os.path.join(PROC_DIR, "options_with_iv.csv"),
...
# Build params dict: params_dict[model_id][loss_id] → np.array of coefficients
params_dict = {}
for _, p in df_params.iterrows():
    m, l = p["model_id"], p["loss_id"]
    params_dict.setdefault(m, {})[l] = np.array(
        [p[n] for n in _dvf.MODEL_SPECS[m]["param_names"]]
    )

COLORS  = {"M0": "#888888", "M1": "#1A73E8", "M2": "#E84040",
           "M3": "#2CA02C", "M4": "#FF7F0E"}
BUCKETS = {"1M": (0.04, 0.12), "3M": (0.12, 0.30),
           "6M": (0.30, 0.60), "12M": (0.60, 1.10)}
# Moneyness grid for DVF curve plotting (predict_sigma receives K/S; log taken internally)
...
# ── Plots 3, 4, 5: one set per estimation loss ────────────────────────────────
for loss in ["L2", "L5"]:
    ...
    for ax, (lbl, (lo, hi)) in zip(axes, BUCKETS.items()):
        ...
        for m, col in COLORS.items():
            # GRID is moneyness; predict_sigma converts to log-moneyness internally
            sig = _dvf.predict_sigma(params_dict[m][loss], GRID,
                                     np.full_like(GRID, T_mid), m)
    ...
    # ── Plot 4: OOS Loss Heatmap ───────────────────────────────────────────────
    # This prevents M0's large absolute loss from collapsing the scale for M1–M4.
    fig, axes = plt.subplots(1, 5, figsize=(16, 3.5))
    for ax, m in zip(axes, _dvf.MODEL_SPECS):
        col = (df_oos[df_oos["model_id"] == m]
        ...
        sns.heatmap(col, ax=ax, annot=True, fmt=".5f", cmap="YlOrRd",
                    ...
                    xticklabels=[loss], yticklabels=["L2", "L5"],
                    vmin=vmin, vmax=vmax)

        # Highlight the matched (est_loss = eval_loss) cell
        matched_row = ["L2", "L5"].index(loss)
        ax.add_patch(plt.Rectangle((0, matched_row), 1, 1,
                                   fill=False, edgecolor="black", lw=2.5))
        ax.set_title(f"Model {m}", fontweight="bold")
        ax.set_xlabel(f"Eval: {loss}")
        ax.set_ylabel("Est Loss" if ax is axes[0] else "")

    loss_label = ("L5 = mean((vega/S)^2 × IV_err^2)" if loss == "L5"
                  else "L2 = mean(IV_err^2)")
    fig.suptitle(
        f"OOS Loss Heatmap — Eval {loss}  (■ = matched est/eval)\n"
        f"{loss_label}",
        y=1.06,
    )
    plt.tight_layout()
    ...
    # ── Plot 5: IV Residuals by moneyness ─────────────────────────────────────
