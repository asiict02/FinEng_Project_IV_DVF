...
def _d2(S, K, T, r, q, σ):
    return _d1(S, K, T, r, q, σ) - σ * np.sqrt(T)
...
def solve_iv(price, S, K, T, r, q, opt, iv_hint=None):
    ...
    # ── Attempt tight bracket from hint ───────────────────────────────────────
    if iv_hint is not None and not np.isnan(iv_hint) and IV_MIN < iv_hint < IV_MAX:
        lo = max(IV_MIN, iv_hint * HINT_LO_MULT)
        hi = min(IV_MAX, iv_hint * HINT_HI_MULT)
        try:
            if f(lo) * f(hi) <= 0:
                iv = brentq(f, lo, hi, xtol=PRICE_TOL, maxiter=200)
                if IV_MIN <= iv <= IV_MAX:
                    return float(iv)
        except Exception:
            pass  # fall through to full bracket

    # ── Full bracket fallback ─────────────────────────────────────────────────
    try:
        if f(IV_MIN) * f(IV_MAX) > 0:
            return np.nan
...
def main():
    ...
    has_hint = "IV_data" in df.columns
    if has_hint:
        print("  IV_data column found — using as warm start for Brent brackets.")
    ...
    pct = 100 * len(df_valid) / len(df)
    print(f"✓ IV solved: {len(df_valid)}/{len(df)} ({pct:.1f}%)")
    print(f"  IV mean={df_valid['IV'].mean():.4f}  std={df_valid['IV'].std():.4f}  "
          f"min={df_valid['IV'].min():.4f}  max={df_valid['IV'].max():.4f}")
    print(f"  Vega mean={df_valid['Vega'].mean():.2f}  "
          f"Vega/S mean={( df_valid['Vega'] / df_valid['S0']).mean():.4f}")
